...
interface BookAirbnbFormProps {
  airbnb: AirBnB;
}

const BookAirbnbForm = ({ airbnb }: BookAirbnbFormProps) => {
  ...
  const form = useForm<z.infer<typeof bookingSchema>>({
    ...
    defaultValues: {
      ...
      propertyName: airbnb?.airBnBType,
    },
  });
  ...
  async function onSubmit(values: z.infer<typeof bookingSchema>) {
    ...
    toast({
      ...
      description:
        "Please be patient, your airbnb reservation is being created!!!",
    });
    try {
      ...
      await sendEmailUser({
        ...
        text: ` Hello ${values.bookerName}, Your AirBnB reservation for ${
          airbnb?.airBnBType
        } located at ${
          airbnb?.airBnBLocation
        } has been successfully reserved for ${new Date(
          values.propertyViewingDate
        ).toDateString()} and we look forward to checking you into the property. If you did not make this reservation, kindly ignore this email. Kind regards`,
      });
      ...
      toast({
        ...
        description:
          "Your Airbnb booking reservation has been successfully created.",
      });
      form.reset();
      ...
      toast({
        title: "Error",
        description: "Failed to create Airbnb reservation. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="flex flex-col pt-7 pb-10 h-full flex-1 w-full items-center justify-center px-4 md:px-6 lg:px-10 bg-slate-100 dark:bg-black">
      <h1 className="capitalize text-center text-amber-600 dark:text-amber-600 my-6 font-poppins font-semibold text-lg md:text-xl xl:text-2xl">
        {" "}
        {airbnb?.airBnBType} {""} Reservation Form
      </h1>
      <Form {...form}>
        <form
