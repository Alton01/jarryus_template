...
const BookPropertyForm = ({ property }: BookPropertyFormProps) => {
  ...
  async function onSubmit(values: z.infer<typeof bookingSchema>) {
    ...
    toast({
      ...
      description:
        "Please be patient, property viewing reservation is being created!!!",
    });
    try {
      ...
      await sendEmailUser({
        ...
        text: ` Hello ${
          values.bookerName
        }, Your viewing inspection reservation for the property on ${
          property.propertyType
        } located at ${
          property.propertyLocation
        } has been successfully reserved for ${new Date(
          values.propertyViewingDate
        ).toDateString()} and we look forward to showing you the property. If you did not make this reservation, kindly ignore this email. Kind regards`,
      });
      ...
      toast({
        ...
        description:
          "Your property viewing reservation has been successfully created.",
      });
      form.reset();
  ...
  return (
    <div className="flex flex-col h-full flex-1 w-full items-center justify-center pt-7 pb-10 px-4 md:px-6 lg:px-10 bg-slate-100 dark:bg-black">
      <h1 className="capitalize text-center text-amber-600 dark:text-amber-600 my-6 font-poppins font-semibold text-lg md:text-xl xl:text-2xl">
        {" "}
        {property?.propertyType} {""}Inspection Booking Form
      </h1>
      <Form {...form}>
        <form
...
export default BookPropertyForm;
